...
if (toggleNextBtn !== null) {
  toggleNextBtn.addEventListener('click', function (e) {
    e.preventDefault(); //roadmap veranderen

    var roadmap = document.querySelector('.roadmap');
    roadmap.style.backgroundPosition = "right";
    toggleNextBtn.style.display = "none"; //.roadmap__stage veranderen
...
window.addEventListener("load", function () {
  if (document.querySelector(".calendar__heading")) {
    ...
    moment.locale("nl");
    var events = [];
    var selectedYear = parseInt(moment().format("YYYY"));
    var selectedMonth = parseInt(moment().format("M"));

    var getLeadingDays = function getLeadingDays(date) {
      var staDay = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 1;
      var result = [];
      var year = date.getFullYear();
      var month = date.getMonth();
      var firstDay = new Date(year, month, 1).getDay();
      var days = firstDay + 7 - (staDay + 7) - 1;
    ...
    var getTrailingDays = function getTrailingDays(leadingDays, monthDays) {
      var result = [];
    ...
    var showCalendar = function showCalendar(year, month, events) {
      ...
      events.forEach(function (event) {
        var compareDate = moment(event.date).format("YYYY-MM-DD");
        var startDate = moment(date).format("YYYY-MM-01");
        var endDate = moment(date).add(1, "month").format("YYYY-MM-01");
      ...
      var daysPrevMonth = getLeadingDays(date);
      var daysThisMonth = getMonthDays(date);
      var daysNextMonth = getTrailingDays(daysPrevMonth, daysThisMonth);
      ...
      daysPrevMonth.forEach(function (day) {
        daysElement.insertAdjacentHTML("beforeend", "<button class=\"day--previous-month\">".concat(day, "</button>"));
      });
      daysThisMonth.forEach(function (day) {
        if (eventDates.includes(day)) {
          var index = eventDates.indexOf(day);
          var event = eventsThisMonth[index];
          var eventName = event.name;
          var eventDate = moment(event.date).format("dddd D MMMM [om] HH:mm");
          daysElement.insertAdjacentHTML("beforeend", "<button class=\"day--this-month\">".concat(day, "<span class=\"dot\"></span><div class=\"calendar__event\"><span class=\"calendar__event__name\">").concat(eventName, "</span><span class=\"calendar__event__date\">").concat(eventDate, "</span></div></button>"));
        } else {
          daysElement.insertAdjacentHTML("beforeend", "<button class=\"day--this-month\">".concat(day, "</button>"));
        }
      });
      daysNextMonth.forEach(function (day) {
        daysElement.insertAdjacentHTML("beforeend", "<button class=\"day--next-month\">".concat(day, "</button>"));
      });
      document.querySelectorAll(".day--this-month").forEach(function (element) {
    ...
    var updateCalendar = function updateCalendar(year, month) {
      document.querySelector(".days").innerHTML = "";
      showCalendar(year, month, events);
    };

    fetch("https://bobstorms.be/jumpstart/calendar.php").then(function (res) {
      return res.json();
    }).then(function (data) {
      var index = 0;
      calendarItems = data.items;
      calendarItems.forEach(function (item) {
        var eventName = item.summary;
        var eventDate = new Date(item.start.dateTime);
        var event = {
          "id": index,
          "name": eventName,
          "date": eventDate
    ...
    document.getElementById("calendar-back").addEventListener("click", function () {
      selectedMonth--;
    ...
    document.getElementById("calendar-forward").addEventListener("click", function () {
      selectedMonth++;
